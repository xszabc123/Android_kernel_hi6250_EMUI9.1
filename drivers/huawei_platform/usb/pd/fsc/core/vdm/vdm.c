...
FSC_S32 processDiscoverSvids(SopType sop, FSC_U32* arr_in, FSC_U32 length_in) {
    ...
    if (__vdmh_in.SVDM.CommandType == INITIATOR) {
		...
		if((__svid_info.num_svids == NUM_SVIDS_12)
			&& (AutoVdmState == AUTO_VDM_DISCOVER_MODES_PP)) {
			AutoVdmState = AUTO_VDM_DISCOVER_SVIDS_PP;
		}

		/* Funky Dongle Handling */
		if(AutoVdmState == AUTO_VDM_DONE) {
			AutoVdmState = AUTO_VDM_DISCOVER_SVIDS_PP;
		}
...
FSC_S32 processDiscoverModes(SopType sop, FSC_U32* arr_in, FSC_U32 length_in) {
    ...
    if (__vdmh_in.SVDM.CommandType == INITIATOR) {
        ...
        vdmm.inform_modes(PolicyState == peDfpVdmModesAcked,sop, __modes_info);
        ExpectingVdmResponse = FALSE;
        platform_set_timer(&VdmTimer, 0);
        VdmTimerStarted = FALSE;
        PolicyState = originalPolicyState;
...
FSC_S32 processAttention(SopType sop, FSC_U32* arr_in, FSC_U32 length_in) {
    ...
    if((__vdmh_in.SVDM.SVID == DP_SID) && (length_in == DP_ATTENTION_WORD_LENGTH)) {
        __attention_status.word = arr_in[DP_ATTENTION_WORD_OFFSET];

        informStatus(__attention_status);
        if((DpPpConfig.word == 0) && (AutoVdmState == AUTO_VDM_DONE)) {
...
void startVdmTimer(FSC_S32 n_pe) {
    ...
    case peDfpVdmModesRequest:
        platform_set_timer(&VdmTimer, tVDMSenderResponse);
        VdmTimerStarted = TRUE;
        break;
    case peDfpVdmModeEntryRequest:
        platform_set_timer(&VdmTimer, tVDMWaitModeEntry);
        VdmTimerStarted = TRUE;
        break;
    case peDfpVdmModeExitRequest:
        platform_set_timer(&VdmTimer, tVDMWaitModeExit);
        VdmTimerStarted = TRUE;
        break;
...
void resetPolicyState (void) {
	...
	case peDfpUfpVdmIdentityRequest:
		...
		PolicyState = originalPolicyState;
		if(AutoVdmState != AUTO_VDM_DONE){
		    AutoVdmState = AUTO_VDM_DISCOVER_ID_PP;
		}
        break;
	case peDfpCblVdmIdentityRequest:
		PolicyState = peDfpCblVdmIdentityNaked;
