...
static FSC_U8                   ProtocolCRC[4];
       ...
       FSC_U8                  g_tries;
       FSC_BOOL                g_collision_lock;
       FSC_BOOL                g_retryfail_lock;
...
void USBPDProtocol(void)
{
    if (Registers.Status.I_HARDRST || Registers.Status.I_HARDSENT)
    {
        ...
        PolicySubIndex = 0;
#ifdef FSC_DEBUG
        StoreUSBPDToken(Registers.Status.I_HARDSENT, pdtHardReset);                                   // Store the hard reset
#endif // FSC_DEBUG
    }
    else
    {
...
void ProtocolGetRxPacket(void)
{
    ...
    if(Registers.Status.RX_EMPTY) return;
    g_crcsent_lock = 1;
    ...
    if((rx_sop != SOP_TYPE_SOP1) && (rx_sop != SOP_TYPE_SOP))
    {
        FSC_PRINT("FUSB %s - Received bad SOP - Likely Rx Error\n", __func__);
        return;
    }
    ...
    /* Crazy edge case resolution */
    if((PolicyRxHeader.NumDataObjects == 0) && (PolicyRxHeader.MessageType == CMTGoodCRC))
    {
        DeviceRead(regFIFO, 4, &ProtocolCRC[0]); // Read out CRC
        ...
        FSC_PRINT("FUSB %s - Triggering Rx Edge Case\n", __func__);
        ProtocolGetRxPacket(); // Get Actual Message
        return;
    }
...
blah:
    if (PolicyRxHeader.NumDataObjects > 0)                                      // Did we receive a data message? If so, we want to retrieve the data
    {
		...
		if(PolicyRxHeader.MessageType == DMTBIST)
		{
			processDMTBIST();
		}
    }
	...
	FSC_PRINT("FUSB %s Rx Buffer [0]: %x [1]: %x [2]: %x\n", __func__, data[0], data[1], data[2]);
    ...
    */
    WriteStateLog(&PDStateLog, dbgGetRxPacket,
                platform_get_system_time(),
                0);

#endif // FSC_DEBUG

	/* For VDM, read FIFO until message is received */
	if(!doublecheck && (PolicyRxHeader.NumDataObjects != 0) && (PolicyRxHeader.MessageType == DMTVenderDefined)
		&&(PolicyRxDataObj[0].SVDM.CommandType == 0))
	{
		...
		{
			FSC_PRINT("FUSB %s VDM message overwritten [0]: %x [1]: %x [2]: %x\n", __func__, data[0], data[1], data[2]);
			ProtocolGetRxPacket();
			g_crcsent_lock = 1;
		}
	}
...
void ProtocolTransmitMessage(void)
{
    ...
    if(PolicyTxSop == SOP_TYPE_SOP) {
        ProtocolLoadSOP();
    }
    else if(PolicyTxSop == SOP_TYPE_SOP1) {
        ProtocolLoadSOP1();
    }
    ...
    FSC_PRINT("FUSB %s, send messge: %d \n",__func__, temp_PolicyTxHeader.MessageType);

    if(DeviceWrite(regFIFO, ProtocolTxBytes, &ProtocolTxBuffer[0]) == FALSE)
    {
        g_collision_lock = 0;
        PDTxStatus = txError;
		FSC_PRINT("FUSB %s Tx FIFO Write Collision\n", __func__);
        return;
    ...
    WriteStateLog(&PDStateLog, dbgSendTxPacket,
                platform_get_system_time(),
                0);
#endif // FSC_DEBUG
}
...
void ProtocolVerifyGoodCRC(void)
{
    ...
    FSC_PRINT("FUSB %s, NumDataObjects=%d, MessageType=%d\n", __func__, PolicyRxHeader.NumDataObjects, PolicyRxHeader.MessageType);
    if ((PolicyRxHeader.NumDataObjects == 0) && (PolicyRxHeader.MessageType == CMTGoodCRC))
    {
        FSC_U8 MIDcompare;
        switch (TokenToSopType(data[0])) {
    ...
    else
    {
		...
		{
			FSC_PRINT("FUSB %s - Triggering Verify GoodCRC Backup Read\n", __func__);
			ProtocolVerifyGoodCRC();
		}
    }
}
...
void ProtocolLoadSOP1(void)
{
    ...
    ProtocolTxBuffer[ProtocolTxBytes++] = SYNC1_TOKEN;                          // Load in the Sync-1 pattern
    ProtocolTxBuffer[ProtocolTxBytes++] = SYNC1_TOKEN;                          // Load in the Sync-1 pattern
    ProtocolTxBuffer[ProtocolTxBytes++] = SYNC3_TOKEN;                          // Load in the Sync-1 pattern
    ProtocolTxBuffer[ProtocolTxBytes++] = SYNC3_TOKEN;                          // Load in the Sync-2 pattern
}
...
{
    manualRetriesEnabled = mode;
}

FSC_U8 getManualRetries(void)
{
    return manualRetriesEnabled;
}
...
{
    if (Registers.Status.I_TXSENT || Registers.Status.I_GCRCSENT || Registers.Status.I_RETRYFAIL)
    FSC_PRINT("FUSB %s, I_TXSENT = %d, I_GCRCSENT=%d. I_RETRYFAIL=%d\n", __func__, Registers.Status.I_TXSENT, Registers.Status.I_GCRCSENT, Registers.Status.I_RETRYFAIL);
    if(g_collision_lock && Registers.Status.I_COLLISION)
    {
        g_collision_lock = 0;
    }
    if(g_retryfail_lock && Registers.Status.I_RETRYFAIL)
    {
        g_retryfail_lock = 0;
    }
    ...
    if(Registers.Status.I_TXSENT)
    {
        ...
        ProtocolVerifyGoodCRC(); /* Sets Success and Idle */
    }
    ...
    if(!g_retryfail_lock)   /* Also set on collision */
    {
        Registers.Status.I_RETRYFAIL = 0;                                   // Clear interrupt

		if(!Registers.Status.RX_EMPTY)
		{
			ProtocolFlushRxFIFO();
		}
		...
		{
			if(!g_tries)                                                          // If no more retries, set as failure
			{
				// Failure :(
				ProtocolState = PRLIdle;                                        // Set the idle state
				PDTxStatus = txError;                                           // Set the transmission status to error to signal the policy engine
			...
			else
			{
				if(g_collision_lock)
				{
					...
					DeviceWrite(regFIFO, ProtocolTxBytes, &ProtocolTxBuffer[0]);        // Reload FIFO
				}
				...
				DeviceWrite(regControl0, 1, &Registers.Control.byte[0]);                // Commit TX_START to the device
				Registers.Control.TX_START = 0;
				platform_set_timer(&ProtocolTimer, 1); /* 3ms */
				g_tries--;
			}
			g_retryfail_lock = 1;
			g_collision_lock = 1;
